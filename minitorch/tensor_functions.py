    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return grad_output.f.mul_zip(grad_output, 
                                     grad_output.f.mul_zip(
                                         grad_output.f.add_zip(grad_output.ones(), grad_output.f.neg_map(dt)), 
                                         dt)), 
...
class Exp(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (dt,) = ctx.saved_values
        return grad_output.f.mul_zip(dt, grad_output)
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        o = order._tensor._storage.astype(int)
        tensor_data = a._tensor.permute(*o)
        ctx.save_for_backward(o)
        return minitorch.Tensor(tensor_data, backend=a.backend)

    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        (data,) = ctx.saved_values
        ndata = [0 for i in data]
        for idx, val in enumerate(data):
            ndata[val] = idx
        return grad_output.permute(*ndata), 0.0
